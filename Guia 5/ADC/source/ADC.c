...
void ADC_Init(ADC_t adc, ADCBits_t reslution, ADCCycles_t cycles) {
	...
	adc->CFG1 = ADC_CFG1_ADIV(0x00);
	ADC_SetResolution(adc, reslution);
...
void ADC_SetCycles(ADC_t adc, ADCCycles_t cycles) {
	if (cycles & ~ADC_CFG2_ADLSTS_MASK) {
		...
		adc->CFG1 |= ADC_CFG1_ADLSMP_MASK;
		adc->CFG2 = (adc->CFG2 & ~ADC_CFG2_ADLSTS_MASK)
				| ADC_CFG2_ADLSTS(cycles);
	}
}
...
bool ADC_Calibrate(ADC_t adc) {
	...
	adc->PG = (0x8000
			| ((adc->CLP0 + adc->CLP1 + adc->CLP2 + adc->CLP3 + adc->CLP4
					+ adc->CLPS) >> (1 + TWO_POW_NUM_OF_CAL)));
	adc->MG = (0x8000
			| ((adc->CLM0 + adc->CLM1 + adc->CLM2 + adc->CLM3 + adc->CLM4
					+ adc->CLMS) >> (1 + TWO_POW_NUM_OF_CAL)));
