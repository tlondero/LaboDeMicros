...
__ISR__ FTM0_IRQHandler(void)
{
	...
	{
		if (status == STATUS_CHF[i])
		{ //Me fijo que canal fue
...
uint8_t FTMInit(uint8_t port, uint8_t num, FTM_DATA data)
{
	...
	if (id != FTM_NO_SPACE)
	{
		...
		FTM_POINTERS[module]->CONTROLS[channel].CnSC = (FTM_POINTERS[module]->CONTROLS[channel].CnSC & ~(FTM_CnSC_MSB_MASK | FTM_CnSC_MSA_MASK)) | (FTM_CnSC_MSB((data.MODE >> 1) & 0X01) | FTM_CnSC_MSA((data.MODE >> 0) & 0X01));
		...
		case FTM_mInputCapture:
			FTM_POINTERS[module]->CONTROLS[channel].CnSC =
				(FTM_POINTERS[module]->CONTROLS[channel].CnSC & ~(FTM_CnSC_ELSB_MASK | FTM_CnSC_ELSA_MASK)) | (FTM_CnSC_ELSB((data.IC_EDGE >> 1) & 0X01) | FTM_CnSC_ELSA((data.IC_EDGE >> 0) & 0X01));
			break;
		case FTM_mOutputCompare:
			FTM_POINTERS[module]->CONTROLS[channel].CnSC =
				(FTM_POINTERS[module]->CONTROLS[channel].CnSC & ~(FTM_CnSC_ELSB_MASK | FTM_CnSC_ELSA_MASK)) | (FTM_CnSC_ELSB((data.OC_EFFECT >> 1) & 0X01) | FTM_CnSC_ELSA((data.OC_EFFECT >> 0) & 0X01));
			FTM_POINTERS[module]->CONTROLS[channel].CnV = FTM_CnV_VAL(data.CNV);
			break;
		case FTM_mPulseWidthModulation:
			FTM_POINTERS[module]->CONTROLS[channel].CnSC =
				(FTM_POINTERS[module]->CONTROLS[channel].CnSC & ~(FTM_CnSC_ELSB_MASK | FTM_CnSC_ELSA_MASK)) | (FTM_CnSC_ELSB((data.EPWM_LOGIC >> 1) & 0X01) | FTM_CnSC_ELSA((data.EPWM_LOGIC >> 0) & 0X01));
...
{
	FTM_POINTERS[FTM_PINOUT[id].MODULE]->CONTROLS[FTM_PINOUT[id].CHANNEL].CnSC = (FTM_POINTERS[FTM_PINOUT[id].MODULE]->CONTROLS[FTM_PINOUT[id].CHANNEL].CnSC & ~FTM_CnSC_CHIE_MASK) | FTM_CnSC_CHIE(mode);
}
