...
const ButtonEvent *ButtonGetEvent(int8_t id)
{
	...
	if (active_buttons[id].new_info)
	{ //if the buttons is enabled
		for (i = 0; i < selector[id]; i++)
		{ // for every state in the GPIO buffer
			if (active_buttons[id].gpio_buffer[i + 1] == active_buttons[id].gpio_buffer[i])
			{ //if they are equal  you could get a typemattic event or a LKP event
				if (active_buttons[id].gpio_buffer[i + 1] == active_buttons[id].active_with)
				{
					time_counter[id] = time_counter[id] + 1;
					if (((time_counter[id] > TYPEMATIC_THRESHOLD) && (time_counter[id] % TYPEMATIC_PERIOD)))
					{										 //evert TYPEMATIC_PERIOD you get a press event if you satisfy the time criteria
			...
			else
			{
				...
				if ((time_counter[id] > 0) && (time_counter[id] < LKP_THRESHOLD) && (active_buttons[id].gpio_buffer[i + 1] == !active_buttons[id].active_with))
...
void ButtonsCheck(void)
{
	...
	for (i = 0; i < MAX_BUTTONS; i++)
	{
		...
		{
			if (active_buttons[i].new_info == false)
			{												 //if there is no new info
