...
#if (DEVELOPMENT_MODE == 1)
	if(pwm_id >= 0 && pwm_id < MAX_PWMS){
		if(freq > 0 && freq < (uint32_t)(1000.0/2.0*(float)PWM_TIMEBASE)){ //no anda si la base de tiempo no es al menos dos veces mas chica que el periodo
			if(dt >= 0 && dt <= 100){
...
void pwm_poll(void){
	...
	for(i=0; i<MAX_PWMS; i++){
		if(INITIALIZED_PWMS[i] == 1){
			if(PWMS[i].queried){				//si la pwm del pin esta activada
				if(gpioRead(PWMS[i].pin)){//si estaba prendido
